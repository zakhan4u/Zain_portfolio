...
const MagicButton = ({title, icon, position, handleClick, otherClasses}:{title : string ; icon: React.ReactNode; position:string ;handleClick?: ()=> void; otherClasses?:string
  ...
  <span className={'inline-flex h-full w-full cursor-pointer items-center justify-center rounded-lg bg-slate-950 px-7 text-sm font-medium text-white gap-4 backdrop-blur-3xl ${otherClasses}'}>
    {position=== 'left' && icon}
   {title}
   {position=== 'right' && icon}
  </span>
